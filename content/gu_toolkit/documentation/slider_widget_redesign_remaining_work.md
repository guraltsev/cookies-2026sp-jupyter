# Slider Widget Redesign: Analysis + Proposal (Remaining Work)

## Context & Goals
The current `SmartFloatSlider` widget is functional but visually heavy: large reset/settings buttons and no play/animate affordance. This design doc proposes UI/UX changes to make the slider feel more “slick,” and adds a clear programmatic path for setting limits and step size.

**Note:** Min/max indicators in the main control row have already been implemented, so this document focuses on the remaining items.
...
Relevant locations:
- Control layout and button sizing: `SmartSlider.py` `__init__`.
...
## UX Issues Identified (Remaining)
...
3. **Slickness & density.** The widget could be more compact and visually modern (more spacing discipline, smaller controls, better alignment).
4. **Programmatic limits/step** are possible but not clearly documented, and no API exists for dynamic per-parameter configuration beyond passing kwargs on `add_param`.
...
- Use consistent alignment baselines between slider, value field, and icon buttons.
...
- Reduce slider/value/button row density (spacing/alignment), and shrink the icon button footprints.
- Add `widgets.Play` (or equivalent play/pause control) and link it to the slider’s `value`.
...
- Settings panel: place `min`/`max`/`step` inputs in a compact row, with consistent label widths and minimal padding.

---

## Open Questions
1. Should the play/animate button be a toggle (play/pause) or a separate `Play` widget?
2. Should animation respect `step` or introduce a separate animation step/interval?
3. Should animation loop (wrap max→min), ping-pong, or stop at max?

---

## Next Steps (Implementation Plan)
1. Update `SmartFloatSlider` control row to shrink buttons and tighten alignment/spacing.
2. Add `Play` control with a default interval and linkage to the slider value; define stop-on-manual-input behavior.
3. Document programmatic configuration in `SmartFigure`/`ParameterManager` docs (kwargs + examples).
